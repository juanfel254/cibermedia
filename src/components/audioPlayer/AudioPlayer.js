...
const AudioPlayer = ({ src }) => {
  ...
  function convertSecs(seconds) {
    return Math.round(seconds % 0x3c).toString();
  }
  function convertHrs(seconds) {
    return Math.floor(seconds / 0xe10).toString();
  }
  function convertMins(seconds) {
    return (Math.floor(seconds / 0x3c) % 0x3c).toString();
  }
      ...
      <div className={styles.audio_player_all_container}>
        <div className={styles.audio_player_container}>
          <div className={styles.audio_player}>
            <div
              className={styles.audio_waveform}
              style={{ paddingRight: waveformWidth + "%" }}
            ></div>
            <img
              src={wavePicture}
              width={800}
          ...
          <div className={styles.audio_controls}>
            <button
              ...
              onClick={
                currentTime === duration ? handleReplay : handlePlayPause
              }
            >
        ...
        <div className={styles.time_controls}>
          ...
          <p className={styles.audio_time}>
            {convertMins(currentTime)}:{convertSecs(currentTime)} /{" "}
            {convertMins(duration)}:{convertSecs(duration)}
          </p>
